...
func TestJWTClaimString(t *testing.T) {
	jwt := makeJWT(t, `{"exp":1893456000,"plan":"tier-123:year:","number":7}`)
	got, ok, err := JWTClaimString(jwt, "plan")
	if err != nil {
		t.Fatalf("JWTClaimString returned error: %v", err)
	}
	if !ok || got != "tier-123:year:" {
		t.Fatalf("JWTClaimString(plan) = (%q, %v), want tier-123:year:, true", got, ok)
	}
	if got, ok, err := JWTClaimString(jwt, "missing"); err != nil || ok || got != "" {
		t.Fatalf("missing claim = (%q, %v, %v), want empty false nil", got, ok, err)
	}
	if got, ok, err := JWTClaimString(jwt, "number"); err != nil || ok || got != "" {
		t.Fatalf("non-string claim = (%q, %v, %v), want empty false nil", got, ok, err)
	}
}
