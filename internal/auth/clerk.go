...
// JWTPayload decodes and returns the JWT payload segment. This is a shape-only
// decode; it does not verify the signature.
func JWTPayload(jwt string) ([]byte, error) {
...
// JWTClaimString returns one string claim from a JWT payload. Missing or
// non-string claims return ("", false, nil). This helper intentionally does
// not verify signatures; callers use it only to mirror non-secret metadata the
// web app already sends in request bodies.
func JWTClaimString(jwt, name string) (string, bool, error) {
	payload, err := JWTPayload(jwt)
	if err != nil {
		return "", false, err
	}
