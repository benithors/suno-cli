...
func TestLooksLikeJWT(t *testing.T) {
	...
	// longRS256Fixture is a realistically-sized JWT shape (header ~86 chars,
	// payload ~430 chars, signature ~344 chars) without credential-like bytes.
	longRS256Fixture := strings.Repeat("h", 86) + "." + strings.Repeat("p", 430) + "." + strings.Repeat("s", 344)
	...
	cases := []struct {
		...
		{"long RS256-shaped JWT", longRS256Fixture, true},
		{"exactly at 150-char floor", atFloor150, true},
		{"realistic minimum HS256", minimalHS256, true},
		{"with Bearer prefix", "Bearer " + longRS256Fixture, true},
...
func TestFindJWTInCookieJar(t *testing.T) {
	...
	longJWT := strings.Repeat("h", 86) + "." + strings.Repeat("p", 430) + "." + strings.Repeat("s", 344)
	...
	cases := []struct {
	...
	}{
		{
			...
			jar:  "__cf_bm=01KRPVRYA2SNQT9BAGD6984WAG_.tt.1; auth_token=" + longJWT + "; csrf=abc123",
			want: longJWT,
		},
		{
			...
			jar:  "auth=Bearer " + longJWT + "; csrf=abc",
			want: longJWT,
		},
		{
