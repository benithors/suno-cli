...
const sunoCaptchaCheckPath = "/api/c/check"
...
const (
	...
	flagQueueOnCaptcha = "queue-on-captcha"
)
...
func captchaRequiredError() error {
	return usageErr(fmt.Errorf(
		"Suno required an hCaptcha token for this generation.\n" +
			...
			"        --token <hcaptcha-token>   (paste a freshly solved browser challenge token)\n" +
			"        --token-stdin              (read the token from stdin; avoids shell history)\n" +
			"        --token-command <cmd>      (run a helper and read the token from stdout)\n" +
			"        --wait-for-gate            (backs off and retries until the gate reopens; --gate-timeout sets the ceiling)\n" +
			"        --queue-on-captcha         (save this request locally, then retry it later with generate queue retry)\n" +
...
func captchaGateEnvelope(queued ...*generationQueueItem) map[string]any {
	out := map[string]any{
		"error_type": "captcha_required",
		"error":      "Suno required an hCaptcha token for this generation",
		"retriable":  true,
		"hint":       "retry with --token, --token-stdin, or --token-command; pass --wait-for-gate; or use --queue-on-captcha to save the request locally",
		"code":       2,
	}
	if len(queued) > 0 && queued[0] != nil {
		out["queued"] = true
		out["queue_id"] = queued[0].ID
		out["retry_hint"] = fmt.Sprintf("suno-pp-cli generate queue retry %s", queued[0].ID)
	}
	return out
}
...
func captchaGateError(cmd *cobra.Command, flags *rootFlags, queued ...*generationQueueItem) error {
	if flags != nil && flags.asJSON {
		_ = json.NewEncoder(cmd.OutOrStdout()).Encode(captchaGateEnvelope(queued...))
	} else if len(queued) > 0 && queued[0] != nil {
		fmt.Fprintf(cmd.ErrOrStderr(), "queued captcha-blocked generation: %s\n", queued[0].ID)
	}
...
type generationCaptchaCheck struct {
	Required       bool `json:"required"`
	CaptchaVersion int  `json:"captcha_version"`
}

func checkGenerationCaptcha(ctx context.Context, c *client.Client) (*generationCaptchaCheck, error) {
	data, _, err := c.Post(ctx, sunoCaptchaCheckPath, map[string]string{"ctype": "generation"})
	if err != nil {
		return nil, err
	}
	var out generationCaptchaCheck
	if err := json.Unmarshal(data, &out); err != nil {
		return nil, fmt.Errorf("parsing generation captcha check: %w", err)
	}
	if out.CaptchaVersion != 2 {
		out.CaptchaVersion = 1
	}
	return &out, nil
}

func prepareGenerationCaptcha(ctx context.Context, c *client.Client, body *sunoGenerateBody) error {
	if body == nil {
		return nil
	}
	check, err := checkGenerationCaptcha(ctx, c)
	if err != nil {
		return err
	}
	if body.Token != nil && body.TokenProvider == nil {
		body.TokenProvider = &check.CaptchaVersion
	}
	if check.Required && body.Token == nil {
		return fmt.Errorf("token_validation_failed: generation captcha check required")
	}
	return nil
}
...
// hydrateGenerationMetadataFromAuth mirrors non-secret request metadata the
// Suno web app sends from the JWT claims. In particular, the web app sets
// metadata.user_tier to the first segment of the JWT "plan" claim.
func hydrateGenerationMetadataFromAuth(body *sunoGenerateBody, cfg *config.Config) {
	if body == nil || cfg == nil || body.Metadata.UserTier != "" || cfg.SunoJwt == "" {
		return
	}
	plan, ok, err := auth.JWTClaimString(cfg.SunoJwt, "plan")
	if err != nil || !ok {
		return
	}
	if i := strings.IndexByte(plan, ':'); i > 0 {
		plan = plan[:i]
	}
	body.Metadata.UserTier = strings.TrimSpace(plan)
}
...
func runGenerationFlow(cmd *cobra.Command, flags *rootFlags, body sunoGenerateBody, wait bool, downloadDir string, workspaceID string) error {
	...
	resp, err := retryOnGate(ctx, cfg, func() (*sunoGenerateResponse, error) {
		...
		hydrateGenerationMetadataFromAuth(&body, c.Config)
		if !flags.dryRun && !cliutil.IsVerifyEnv() {
			if perr := prepareGenerationCaptcha(ctx, c, &body); perr != nil {
				return nil, perr
			}
		}
	...
	if err != nil {
		if isCaptchaRequired(err) {
			queueOnCaptcha, _ := cmd.Flags().GetBool(flagQueueOnCaptcha)
			if queueOnCaptcha {
				item, qerr := queueGenerationOnCaptcha(cmd, body, wait, downloadDir, workspaceID, err)
				if qerr != nil {
					return fmt.Errorf("queueing captcha-blocked generation: %w", qerr)
				}
				return captchaGateError(cmd, flags, item)
			}
