...
func newGenerateCmd(flags *rootFlags) *cobra.Command {
	...
	cmd.PersistentFlags().Bool(flagQueueOnCaptcha, false, "Save a challenged generation locally for later retry instead of dropping the request")
	...
	cmd.AddCommand(newSunoGenerationQueueCmd(flags))
