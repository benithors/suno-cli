...
Resource scoping:
  ...
  suno-pp-cli sync --resources channels,messages

  # Full resync (ignore previous checkpoint)
  suno-pp-cli sync --full

  # Incremental sync: only records from the last 7 days
  suno-pp-cli sync --since 7d

  # Parallel sync with 8 workers
  suno-pp-cli sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  suno-pp-cli sync --latest-only`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			// Dry-run / verify short-circuit BEFORE the auth gate so
			// `sync --dry-run` is reachable without credentials (verify and
			// narrative-example checks rely on this).
			if dryRunOK(flags) {
				fmt.Fprintln(cmd.OutOrStdout(), "would sync resources to the local store")
				return nil
			}

			// Hand-built auth gate: fail fast with a clear auth error rather
			// than dialing out (or, under verify mode, short-circuiting
			// silently) when no credential is configured.
			if authed, aerr := sunoAuthConfigured(flags.configPath); aerr != nil {
				return configErr(aerr)
			} else if !authed {
				return authErr(fmt.Errorf("not authenticated; run 'suno-pp-cli auth login' or set SUNO_JWT"))
			}
			...
			// Build the clip↔workspace membership index from each synced
			// project's clip list. Best-effort post-pass: failures warn but
			// never fail the overall sync.
			for _, resource := range resources {
				if resource != "workspace" {
					continue
				}
				if n, merr := syncWorkspaceMembership(cmd.Context(), c, db); merr != nil {
					if humanFriendly {
						fmt.Fprintf(os.Stderr, "warning: workspace membership index incomplete: %v\n", merr)
					}
				} else if humanFriendly && n > 0 {
					fmt.Fprintf(os.Stderr, "  workspace membership: indexed %d project(s)\n", n)
				}
				break
			}
	...
	cmd.Flags().StringVar(&dbPath, "db", "", "Database path (default: ~/.local/share/suno-pp-cli/data.db)")
...
func upsertSingleObject(db *store.Store, resource string, data json.RawMessage) error {
	...
	case "clips":
		return db.UpsertClips(data)
	case "lyrics":
		return db.UpsertLyrics(data)
	case "personas":
		return db.UpsertPersonas(data)
	case "workspace":
		return db.UpsertWorkspace(data)
	default:
