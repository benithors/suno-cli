...
In local mode: searches locally synced data only.`,
  ...
  suno-pp-cli search "error timeout"

  # Force local search only
  suno-pp-cli search "payment failed" --data-source local

  # Search a specific resource type locally
  suno-pp-cli search "critical" --type transactions --data-source local

  # JSON output for piping
  suno-pp-cli search "critical" --json --limit 20`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if err != nil {
				return fmt.Errorf("opening local database: %w\nRun 'suno-pp-cli sync' first to populate the local database.", err)
			}
			defer db.Close()
			...
			switch resourceType {
			case "clips":
				results, err = db.SearchClips(query, limit)
			case "lyrics":
				results, err = db.SearchLyrics(query, limit)
			case "personas":
				results, err = db.SearchPersonas(query, limit)
			case "workspace":
				results, err = db.SearchWorkspace(query, limit)
			case "":
				...
				{
					partial, searchErr := db.SearchClips(query, limit)
					if searchErr != nil {
						return fmt.Errorf("search clips failed: %w", searchErr)
					}
				...
				{
					partial, searchErr := db.SearchLyrics(query, limit)
					if searchErr != nil {
						return fmt.Errorf("search lyrics failed: %w", searchErr)
					}
					for _, r := range partial {
						key := string(r)
						if !seen[key] {
				...
				{
					partial, searchErr := db.SearchPersonas(query, limit)
					if searchErr != nil {
						return fmt.Errorf("search personas failed: %w", searchErr)
					}
					for _, r := range partial {
						key := string(r)
						if !seen[key] {
				...
				{
					partial, searchErr := db.SearchWorkspace(query, limit)
					if searchErr != nil {
						return fmt.Errorf("search workspace failed: %w", searchErr)
					}
					for _, r := range partial {
						key := string(r)
						if !seen[key] {
