...
func TestBuildGenerateBody_SimpleModeUsesGPTDescriptionPrompt(t *testing.T) {
	body := buildGenerateBody(generateInput{
		createMode:           "simple",
		mv:                   "chirp-fenix",
		gptDescriptionPrompt: "short upbeat synth pop verification cue",
	})
	if body.Metadata.CreateMode != "simple" {
		t.Errorf("create_mode = %q, want simple", body.Metadata.CreateMode)
	}
	if body.Prompt != "" {
		t.Errorf("prompt = %q, want empty string for simple mode", body.Prompt)
	}
	if body.GPTDescriptionPrompt != "short upbeat synth pop verification cue" {
		t.Errorf("gpt_description_prompt = %q", body.GPTDescriptionPrompt)
	}
	if body.Metadata.LyricsModel != "default" {
		t.Errorf("lyrics_model = %q, want default", body.Metadata.LyricsModel)
	}
	raw, err := json.Marshal(body)
	if err != nil {
		t.Fatalf("marshal: %v", err)
	...
	for _, want := range []string{
		`"create_mode":"simple"`,
		`"prompt":""`,
		`"gpt_description_prompt":"short upbeat synth pop verification cue"`,
		`"lyrics_model":"default"`,
		`"user_uploaded_images_b64":null`,
	} {
		if !strings.Contains(js, want) {
			t.Errorf("serialized body missing %s: %s", want, js)
		}
	}
	for _, notWant := range []string{`"title":`, `"tags":`, `"negative_tags":`, `"token":`} {
		if strings.Contains(js, notWant) {
			t.Errorf("simple-mode body should not include %s when unset: %s", notWant, js)
		}
	}
}
...
func TestBuildGenerateBody_TitlePreservedAndReferencesNull(t *testing.T) {
	...
	if !strings.Contains(js, `"artist_clip_id":null`) {
		t.Errorf("artist_clip_id should remain null when absent, got: %s", js)
	}
	if !strings.Contains(js, `"token_provider":null`) {
		t.Errorf("token_provider should be null when no token provider is known, got: %s", js)
	}
}

func TestBuildGenerateBody_TokenProvider(t *testing.T) {
	provider := 2
	body := buildGenerateBody(generateInput{
		createMode:    "custom",
		mv:            "chirp-fenix",
	...
	js := string(raw)
	if !strings.Contains(js, `"token":"challenge-token"`) {
		t.Errorf("serialized body missing token: %s", js)
	}
	if !strings.Contains(js, `"token_provider":2`) {
		t.Errorf("serialized body missing token_provider 2: %s", js)
	}
}
